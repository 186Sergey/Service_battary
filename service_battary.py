...
battary_brend = ("6СТ-60", "6СТ-62", "6СТ-66", "6СТ-75", "6СТ-90", "6СТ-100", "6СТ-110",
                 "6СТ-132", "6СТ-140", "6СТ-180", "6СТ-190", "6СТ-192", "6СТ-225")


density_up_to = ("1,14 г/см3", "1,15 г/см3", "1,16 г/см3", "1,17 г/см3", "1,18 г/см3", "1,19 г/см3", "1,20 г/см3", 
                 "1,12 г/см3", "1,22 г/см3", "1,23 г/см3", "1,24 г/см3", "1,25 г/см3", "1,26 г/см3", "1,27 г/см3",
                 "1,28 г/см3", "1,29 г/см3")
...
class Main(tk.Frame):
    ...
    def records(self, my_data, gos_nomer, brend_auto, brend_battary, kol_battary, description, density_do, density_posle):
        self.db.insert_data(my_data, gos_nomer, brend_auto, brend_battary, kol_battary, description, density_do, density_posle)
        self.view_records()

    def update_record(self, my_data, gos_nomer, brend_auto, brend_battary, kol_battary, description, density_do, density_posle):
        self.db.c.execute('''UPDATE battary SET my_data=?, gos_nomer=?, brend_auto=?, brend_battary=?, kol_battary=?, 
                                                description=?, density_do=?, density_posle=? WHERE ID=?''',
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        self.my_data_combo = ttk.Combobox(self, values=my_data, state="readonly")
        ...
        self.btn_ok.bind('<Button-1>', lambda event: self.view.records(self.my_data_combo.get(), self.entry_gos_nomer.get(), self.brend_auto_entry.get(),
                                                                       self.brend_battary_combobox.get(), self.kol_battary_spinbox.get(), self.description.get(),
                                                                       self.density_do_combobox.get(), self.density_posle.get()
                                                                       ))
...
class Update(Child):
    ...
    def init_edit(self):
        ...
        btn_edit.bind('<Button-1>', lambda event: self.view.update_record(self.my_data_combo.get(), self.entry_gos_nomer.get(), self.brend_auto_entry.get(),
                                                                          self.brend_battary_combobox.get(), self.kol_battary_spinbox.get(), self.description.get(),
                                                                          self.density_do_combobox.get(), self.density_posle.get(),))
    ...
    def default_data(self):
        ...
        self.brend_auto_entry.insert(0, row[3])
...
class DB:
    def __init__(self):
        ...
        self.c.execute(
            '''CREATE TABLE IF NOT EXISTS battary (id integer primary key, my_data TEXT, gos_nomer TEXT, brend_auto TEXT, brend_battary TEXT, 
                                                   kol_battary TEXT, description TEXT, density_do TEXT, density_posle TEXT)''')
        self.conn.commit()
